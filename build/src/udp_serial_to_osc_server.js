...
exports.newUDPServer = void 0;
const dgram = require("node:dgram");
const ip = require("ip");
const osc_utils = require("./utils/osc_util");
...
const serial_to_osc_router_client = osc_utils.newOscClient(local_ip, osc_router_port);
// ----------------------------------------------------------------------------------
// all other osc related stuff
// ----------------------------------------------------------------------------------
// serial-to-udp server stuff
serial_server.bind(multicast_port);
function newUDPServer() {
    return dgram.createSocket({
        type: 'udp4',
        reuseAddr: true, // for testing multiple instances on localhost
    });
}
exports.newUDPServer = newUDPServer;
serial_server.on('message', (msg, rinfo) => {
    // console.log(msg.toString().trim());
    console.log(`server got: ${msg} from ${rinfo.address}:${rinfo.port}`);
    // const parsed_msg =
    // Echo message back to client
    serial_server.send('I got a message from you: ' + msg, rinfo.port, rinfo.address);
    ...
    osc_utils.parseMessageAndSendOSCMessage(msg.toString(), serial_to_osc_router_client);
});
serial_server.on('listening', function () {
...
//# sourceMappingURL=udp_serial_to_osc_server.js.map